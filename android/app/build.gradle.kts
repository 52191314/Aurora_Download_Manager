...
android {
    ...
    buildTypes {
        release {
            // OSS edition: no Play upload keystore — fall back to the debug
            // keystore so release APKs build anywhere. F-Droid re-signs with
            // its own key when building from source; GitHub Releases can ship
            // debug-signed APKs (standard for open-source Android apps).
...
dependencies {
    ...
    // OSS edition: Play Feature Delivery (com.google.android.play:*) was
    // removed — no on-demand modules, everything ships in the fat APK.
